...
export class Worker {
     ...
     */
    stop() {
        return Promise.all(__classPrivateFieldGet(this, _servers).map(server => server.close()));
    }
    /**
     ...
     */
    start(servers = []) {
        __classPrivateFieldSet(this, _servers, servers.map(options => new Server(options)));
        const id = cluster.worker.id;
        const logger = Application.instance.logger;
        return Promise.all(__classPrivateFieldGet(this, _servers).map(server => server
    ...
    _extractUserFromRequest(authorization) {
        if (authorization == undefined || !authorization.length)
            return '-';
        try {
