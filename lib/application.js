...
export class Application {
    ...
    async init(args = {}) {
        if (typeof args.config == 'string')
            this.servers = await Application._parseConfiguration(await promises.readFile(args.config, 'utf8'));
        else
    ...
    async run(args = process.argv) {
        if (cluster.isWorker) {
            ...
            if (!program.config && !program.target)
                program.help();
            if (program.silent)
                this.logger = new Console(new Writable({
    ...
    async _parseConfiguration(config) {
        ...
        if (!data.length)
            throw new TypeError('Invalid configuration data');
        const isJson = (data.startsWith('[') && data.endsWith(']')) || (data.startsWith('{') && data.endsWith('}'));
        let servers;
        if (!isJson)
            servers = safeLoadAll(data);
        else {
            servers = JSON.parse(data);
            if (!Array.isArray(servers))
                servers = [servers];
        }
        if (!servers.every(value => typeof value == 'object' && Boolean(value)))
            throw new TypeError('Invalid configuration format');
        for (const options of servers) {
            if (!('routes' in options) && !('target' in options))
                throw new TypeError('You must provide at least a target or a routing table');
            ...
            if (typeof options.ssl == 'object' && options.ssl) {
                ...
                for (const key of keys)
                    options.ssl[key] = await promises.readFile(options.ssl[key]);
            }
        }
    ...
    async _startWorkers() {
        ...
        if (program.target)
            ...
            if (!servers.length)
                throw new Error('Unable to find any configuration for the reverse proxy');
        }
        const workers = Math.min(Math.max(1, program.workers), cpus().length);
        for (let i = 0; i < workers; i++)
            cluster.fork().send({ action: 'start', params: servers });
    }
}
/** The default number of workers. */
Application.defaultWorkerCount = Math.ceil(cpus().length / 2);
/** The format used for logging the requests. */
Application.logFormat = ':req[host] :remote-addr - :remote-user [:date[iso` ":method :url HTTP/:http-version" :status :res[content-length] ":referrer" ":user-agent"';
