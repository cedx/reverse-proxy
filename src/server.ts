...
export class Server extends EventEmitter {
	...
	_proxyOptions?: httpProxy.ServerOptions;
	...
	_sslOptions?: https.ServerOptions;
	...
	constructor(options: ServerOptions = {}) {
		...
		if (target) this.routes.set("*", Route.from(target));
	}
	 ...
	 */
	_getHostname(req: http.IncomingMessage): string {
	 ...
	 */
	_onHttpRequest(req: http.IncomingMessage, res: http.ServerResponse) {
	 ...
	 * @param res The response sent by the server.
	 */
	_onRequestError(err: Error, req: http.IncomingMessage, res: http.ServerResponse) {
		this.emit(Server.eventError, err);
	 ...
	 */
	_onWebSocketRequest(req: http.IncomingMessage, socket: Socket, head: Buffer) {
		const hostname = this._getHostname(req);
		const pattern = this.routes.has(hostname) ? hostname : "*";
	 ...
	 */
	_sendStatus(res: http.ServerResponse, statusCode: number) {
